"""Pool v5: another ~450 never-seen decks (round 23).
...
POOL_V5 = Path(__file__).resolve().parent / "decks" / "pool_v5"

SOURCES = [
    ("res/quest/duels", 260),
    ("res/decks/legends", 200),
    ("res/geneticaidecks", 200),
    ("res/quest/precons", 120),
    ("res/starter", 60),
]
...
def main():
    ...
    client = sim_server.shared_client("poolv5")
    kept = len(list(POOL_V5.glob("*.dck")))
    while_start = kept
    for rel, f, text in candidates:
        if kept >= args.target:
            break
        dck = f"poolv5_{kept:03d}"
        ...
        if len(re.findall(r"Game Result", out)) == 1:
            (POOL_V5 / f"{dck}.dck").write_text(text2, encoding="utf-8")
            kept += 1
            if kept % 25 == 0:
    ...
    client.close()
    print(f"pool v5 complete: +{kept - while_start} "
