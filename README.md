...
The site currently leads with the **card embedding map**
(`index.html`, generated by `experiments/make_map.py` — see
`experiments/README.md` for the research track): every playable Magic card
as a point in rules-text embedding space.

The **deck builder** is shelved but fully functional at `/builder.html`
(`app.js`, `style.css`): it runs the same Python engine in your browser via
[Pyodide](https://pyodide.org) (Python compiled to WebAssembly) — there is
no server. Paste or upload your collection; the page looks up just those
cards through the [Scryfall API](https://scryfall.com/docs/api) and builds
the deck locally. To make it the front page again, regenerate/rename:
the map and builder are independent static pages.
